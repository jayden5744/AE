...
def main():
    data_fname = './kakao_data/train.json'
    meta_fname = './kakao_data/meta.pkl'
    ...
    for epoch in range(num_epochs):
        ...
        for pl in tqdm(train_loader):
            ...
            if noise_prob > 0:
                ...
                output, mu, var = model(inputs_noise)
            else:
                output, mu, var = model(inputs)
            loss, num_song_tags = loss_function(output, inputs, mu, var)
            # backward
            optimizer.zero_grad()
